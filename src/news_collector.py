...
class FinTechNewsAnalyzer:
    ...
    # 금융IT 관련 키워드 (기사 필터링용) - 넓게 수집
    KEYWORDS = [
        # 금융 기관 (넓게)
        "은행", "증권", "보험", "카드사", "저축은행", "캐피탈",
        "금융사", "금융회사", "금융권", "금융업", "금융기관",
        # 주요 금융사
...
[관련있음 - 이런 기사는 통과]
- 은행/증권/보험사 관련 뉴스 (정책, 실적, 서비스 등)
- 금융 정책, 금융 규제 (주담대, RWA, 자본규제, 금리 등)
- 핀테크, 오픈뱅킹, 마이데이터, 간편결제
- 금융 보안, 금융 사기 방지, 보이스피싱
- 금융권 채용, 인사, 조직 변화
- 금융결제원, 코스콤, 금감원, 금융위, 한국은행 관련
- 가상자산, 코인 거래소, 디지털자산 규제
- 금융사의 IT 투자, 디지털 전환, AI 도입

[관련없음 - 이런 기사만 제외]
- 전기차, 배터리, 반도체, 방산, 제조업 (금융과 무관한 산업)
- 해외 빅테크(구글, MS, 아마존, 메타) 자체 실적/투자 뉴스
- 단순 주가/환율/지수 시세 정보 (코스피 몇 포인트 등)
- 연예, 스포츠, 날씨 등 완전히 다른 분야

기사 제목: {title}
기사 요약: {summary}

판단이 애매하면 "관련있음"으로 해줘.
"관련있음" 또는 "관련없음"으로만 답변해."""
